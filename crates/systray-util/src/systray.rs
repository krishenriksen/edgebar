// Add these macro definitions at the top of the file:
#[cfg(debug_assertions)]
macro_rules! debug_info {
    ($($arg:tt)*) => { debug_info!($($arg)*); }
}
...
impl Systray {
  ...
  fn on_event(&mut self, event: TrayEvent) -> Option<SystrayEvent> {
    match &event {
      TrayEvent::IconAdd(icon_data) | TrayEvent::IconUpdate(icon_data) => {
        debug_info!("Icon modified or added: {:?}", icon_data);
      ...
      TrayEvent::IconRemove(icon_data) => {
        debug_info!("Icon removed: {:?}", icon_data);
  ...
  pub fn send_action(
    ...
    debug_info!("Sending icon action: {:?}", self.icons);
